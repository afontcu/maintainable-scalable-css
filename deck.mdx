...
```notes
El primer problema con CSS es que nunca nos habiamos parado a entender qué estaba 
pasando. Escribiamos en un lenguaje del que sabíamos cómo escribirlo, pero no
lo entendíamos. Ni box model, ni cascada, ni especificidad ni nada de nada.
Eramos como cabras locas, habeis visto nunca a una cabra loca?

Y es lo mismo en cualquier lenguaje. Puedes usar CSS sin entender la cascada
igual que puedes usar Javascript sin entender scoping o las closures, pero
vas a tener problemas cuando las cosas se salgan del camino habitual.

Y esto no significa que JS sea un mal lenguaje ni que CSS sea un desastre
incomprensible. Significa que hay que leer documentación, tutoriales, ejemplos,
y aprender.


El segundo problema con CSS es que esperábamos (nosotros y todo el mundo) que
fuera sencillo. Total, solamente sirve para dar colores a una aplicación, no
entiendo como puede costar tanto. Y el problema, como siempre, son las expectativas.
No que sea fácil o dificil, sino la expectativa de "esto tiene que ser facil".
...
necesitas, no antes. Por eso "-first", y no "-only".
```
---
```jsx
// Nav.js
...
<ul>
  <Appear>
    <li>Use software principles, even in CSS. Understand the tool.</li>
    <li>Think in terms of Design Systems and Components.</li>
    <li>Forget about content-based class naming.</li>
    <li>Use utilities and abstract out components as needed.</li>
    <li>Agree on structure, naming, principles, and linting.</li>
  </Appear>
</ul>
